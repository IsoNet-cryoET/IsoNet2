...
+ 下载安装包，在Linux环境下进行安装:
  ...
  + [Miniconda](https://www.anaconda.com/docs/getting-started/miniconda/main)(新手推荐，安装包体积小).
...
+ 在终端中执行安装脚本（以 Miniconda 为例）：  
   ```bash
   bash Miniconda3-latest-Linux-x86_64.sh
   ```
+ 根据提示一路回车即可。如果某些选项不确定，可以暂时接受默认设置，之后仍可修改。  

+ 安装结束后，关闭当前终端并重新打开一个新的终端。  
...
# 1.4 安装 CUDA (https://docs.nvidia.com/cuda/cuda-installation-guide-linux/index.html)

+ 在 Linux 下查看你的显卡型号，例如：  
   ```bash
   nvidia-smi
   ```  
   或者使用系统设置。  
...
+ 从 CUDA Toolkit Archive (https://developer.nvidia.com/cuda-toolkit-archive) 选择一个与你驱动兼容、且版本 ≥ 11.8 的 **CUDA Toolkit**，按文档步骤安装。  
...
其余与显微镜相关的参数（如 Cs、电压、振幅对比度等），使用默认值。
...
> 建议在 `tomograms.star` 中根据 0° tilt 时的 欠焦（defocus）估值，手动修改 **rlnDefocus** 一列，使每个 tomogram 拥有合理的 defocus 值。
...
**subtomo_size** 可以是16的倍数，但必须大于64. The 默认选项 (96) 是足够训练出一个好的模型, 但如果显存足够的话，你可以用更大的subtomo_size，可能可以提升训练模型的效果。
...
打开 **Jobs Viewer** tab. 我们可以看到所有的 jobs' 状态 and IDs. 你可以终止选定的任务。
