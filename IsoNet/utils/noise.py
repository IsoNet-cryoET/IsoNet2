...
def get_noise(noise_dir=None, cube_size=64, noise_mode = None):
    '''
    crop out one subtomo and missing wedge simulated one from input data,
    and save them as train set
    '''
    if noise_dir is not None:
        path_noise = sorted([noise_dir+'/'+f for f in os.listdir(noise_dir)])
        path_index = np.random.randint(len(path_noise))
        noise_volume, _ = read_mrc(path_noise[path_index])
    else:
        from IsoNet.utils.noise import make_noise_one
        noise_volume = make_noise_one(cubesize = cube_size,mode=noise_mode)
