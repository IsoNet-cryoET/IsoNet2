...
args = sys.argv
from IsoNet.utils.mask import maxmask,stdmask
import numpy as np
#import cupy as cp
import os
...
def make_mask(tomo_path, mask_name, mask_boundary=None, side=5, density_percentage=50., std_percentage=50., surface=None):
    ...
    # Estimate total steps: load + resize + gaussian + mask1 + mask2 + combine + boundary + surface + upsample + write
    total_steps = 6
    with tqdm(total=total_steps, desc='Masking Tomogram', unit=' step') as pbar:
        sp = np.array(tomo.shape)
        sp2 = sp // 2
        bintomo = resize(tomo, sp2, anti_aliasing=True)
        pbar.update(1)  # resize done
    
        gauss = gaussian_filter(bintomo, side / 2)
        pbar.update(1)  # gaussian done
        
        if density_percentage <= 99.8:
            mask1 = maxmask(gauss, side=side, percentile=density_percentage)
        else:
            mask1 = np.ones(sp2)
        pbar.update(1)  # mask1 done

        if std_percentage <= 99.8:
            mask2 = stdmask(gauss, side=side, threshold=std_percentage)
        else:
            mask2 = np.ones(sp2)
        pbar.update(1)  # mask2 done

        out_mask_bin = np.multiply(mask1, mask2)
        
        if mask_boundary is not None:
            from IsoNet.utils.mask import boundary_mask
